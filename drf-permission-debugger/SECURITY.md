...
- The version of `drf-permission-debugger` affected
...
- Pin `drf-permission-debugger` to a specific version range in
...
- Leave `RESTRICT_TO_STAFF` at its default (`True`) in production -
  disabling it exposes your permission stack's internal names and
  denial reasons to every requester, not just trusted staff. See
  `docs/security.md`.
