...
```bash
git clone https://github.com/mahmoudgshaker/drf-permission-debugger.git
cd drf-permission-debugger
python -m venv .venv
...
1. Open an issue first for anything beyond a trivial fix, so the
   ...
   especially important for anything touching `check_permissions`/
   `check_object_permissions` in `mixins.py`, since this package's core
   promise is that it **never changes DRF's own authorization
   behavior** - any change there must preserve the exact grant/deny
   outcome and check order a plain `APIView` would produce.
2. Fork the repository and create a branch from `main`:
   `git checkout -b feat/short-description`.
3. Write tests for your change against real permission classes and
   real HTTP requests via `rest_framework.test.APIClient` - include a
   test proving the mixin's behavior is identical to the equivalent
   undecorated view for both the granted and denied case.
...
Open a [GitHub Discussion](https://github.com/mahmoudgshaker/drf-permission-debugger/discussions)
