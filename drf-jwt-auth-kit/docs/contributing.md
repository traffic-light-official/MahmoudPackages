# Contributing to drf-jwt-auth-kit
...
This project follows the [Code of Conduct](https://github.com/MahmoudGShake/MahmoudPackages/blob/master/drf-jwt-auth-kit/CODE_OF_CONDUCT.md). By
...
cd MahmoudPackages/drf-jwt-auth-kit
python -m venv .venv
...
1. Open an issue first for anything beyond a trivial fix, so the approach
   ...
   for anything touching token rotation, reuse detection, or CSRF
   validation, since correctness there is security-critical.
...
Any change to `src/drf_jwt_auth_kit/models.py` requires a matching
migration generated with `python manage.py makemigrations
drf_jwt_auth_kit` (using the test settings module,
`tests.test_app.settings`), committed alongside the model change.
