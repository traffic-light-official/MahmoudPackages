...
This project follows the [Code of Conduct](https://github.com/MahmoudGShake/MahmoudPackages/blob/master/drf-serializer-performance-profiler/CODE_OF_CONDUCT.md). By
...
1. Open an issue first for anything beyond a trivial fix, so the
   ...
   **especially** important for anything touching
   `to_representation()` in `mixins.py`, since this package's entire
   value depends on it staying byte-for-byte equivalent to DRF's own
   `Serializer.to_representation()` (see `docs/architecture.md`).
...
3. Write tests for your change proving both the profiling data is
   ...
   `tests/test_mixins.py::TestOutputIsUnchanged` for the pattern.
   Assert query counts exactly (deterministic); avoid asserting exact
   timing values (machine-dependent) - see `docs/testing.md`.
