# Contributing to drf-serializer-performance-profiler
...
```bash
git clone https://github.com/mahmoudgshaker/drf-serializer-performance-profiler.git
cd drf-serializer-performance-profiler
python -m venv .venv
...
1. Open an issue first for anything beyond a trivial fix, so the
   ...
   especially important for anything touching `to_representation` in
   `mixins.py`, since this package's core promise is that it **never
   changes the serialized output** - any change there must produce
   byte-for-byte identical `.data` to the same serializer without the
   mixin.
2. Fork the repository and create a branch from `main`:
   `git checkout -b feat/short-description`.
3. Write tests for your change proving both the profiling data is
   correct AND the serialized output is unchanged - see
   `tests/test_integration.py::TestOutputIsUnchanged` for the pattern.
...
Open a [GitHub Discussion](https://github.com/mahmoudgshaker/drf-serializer-performance-profiler/discussions)
